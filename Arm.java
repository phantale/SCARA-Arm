...
public class Arm
{
    ...
    public void directKinematic(){
        ...
        double  xa = (xj1+xj2)*0.5;
        double  ya = (yj1+yj2)*0.5;
        // distance between joints
        double d = xj2-xj1;
        if (d<2*r){
            valid_state = true;
            // half distance between tool positions
            double  h = Math.sqrt((r*r)-((ya-yj1)*(ya-yj1) +(xa-xj1)*(xa-xj1)));
            double alpha= Math.atan((xj2-xj1)/(yj2-yj1));
            // tool position
    ...
    public void inverseKinematic(double xt_new,double yt_new){
        ...
        double alpha1 = Math.atan((xt-xm1)/(yt-ym1));
        double xa1 = (xt+xm1)*0.5;
        double ya1 = (yt+ym1)*0.5;
        // elbows positions
    ...
    public int get_pwm1(){
        int pwm = 0;
        pwm = (int)((-10.784*theta1) +551);
        return pwm;
    }
    // ditto for motor 2
    public int get_pwm2(){
        int pwm =0;
        pwm = (int)((-10.758*theta2)  +683);
        return pwm;
    }
